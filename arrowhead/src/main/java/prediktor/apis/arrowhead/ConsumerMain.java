...
package prediktor.apis.arrowhead;
...
import prediktor.apis.arrowhead.command.ReadAllItemsCommand;
import prediktor.apis.arrowhead.command.ReadAllItemsNameCommand;
import prediktor.apis.arrowhead.command.ReadItemsCommand;
import prediktor.apis.arrowhead.command.WriteItemsCommand;
import prediktor.apis.arrowhead.service.ApisServiceConsumerREST_WS;
...
public class ConsumerMain {
  ...
  private ServiceRequestForm compileSRF() {
     ...
     */
    ArrowheadService service = new ArrowheadService("ApisPullService", Collections.singleton("JSON"), metadata);
  ...
  private void consumeService(String providerUrl) {
	   ...
	   System.out.println("Endpoint: " + providerUrl);
	   HttpEndpoint endpoint = HttpEndpoint.createFromString(providerUrl);
	   
		String host = endpoint.getHost();
		if(host.endsWith("."))
			endpoint.setHost(host.substring(0, host.length() - 1));
		
		
		ClientFactoryREST_WS clientFactoryREST_WS = new ClientFactoryREST_WS("cacerts.jks", "changeit", "tester.jks", "changeit");
		clientFactoryREST_WS.setAcceptMismatchedHostnames(true);
		ApisServiceConsumerREST_WS consumer = new ApisServiceConsumerREST_WS(endpoint, clientFactoryREST_WS);
		
		Shell shell = new Shell("apisclient");
		shell.registerCommand(new ReadAllItemsCommand(consumer));
		shell.registerCommand(new ReadAllItemsNameCommand(consumer));
		shell.registerCommand(new ReadItemsCommand(consumer));
		shell.registerCommand(new WriteItemsCommand(consumer));
		shell.run();
  ...
  private String sendOrchestrationRequest(ServiceRequestForm srf) {
    ...
    System.out.println("Using service from module: " + provider.getSystemName());
